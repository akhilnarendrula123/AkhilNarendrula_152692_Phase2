...
public class Client {
	...
	private static void login(Customer customerBean) {
		...
		do {
			...
			case 3:
				...
				if (service.depositAmount(depositAmount, mobNum1)) {
					...
					System.out.println("Updated Balance is " + bean3.getBalance());
				} else
				...
				try {
					try {
						if (service.fundTransfer(senderMobNum, receiverMobNum, transferAmount)) {
							System.out
									.println("Rs. " + transferAmount + " Succesfully Transfered to " + receiverMobNum);
			...
			case 6:
				System.out.println(
						"_____________________________Logged Out Successfully________________________________");
		...
		} while (choiceLogin >= 1 && choiceLogin <= 6);
		scanner.close();
	}
}
